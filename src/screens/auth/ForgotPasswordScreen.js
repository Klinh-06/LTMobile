...
export default function ForgotPasswordScreen({ navigation }) {
  ...
  const [step, setStep] = useState(1); // 1: enter phone, 2: enter new password, 3: success
  ...
  const handleCheckPhone = async () => {
    ...
    if (!phone) { Alert.alert('Lỗi', 'Vui lòng nhập số điện thoại'); return; }
    if (!phoneRegex.test(phone.trim())) {
      Alert.alert('Lỗi', 'Số điện thoại không hợp lệ');
      return;
    }
    // Demo account cannot reset
    if (phone.trim() === '0901234567') {
      Alert.alert('Không thể đặt lại', 'Tài khoản demo không hỗ trợ đặt lại mật khẩu.');
      return;
    }
    const users = await getRegisteredUsers().catch(() => []);
    const found = users.find(u => u.phone === phone.trim());
    if (!found) {
      Alert.alert('Không tìm thấy', 'Số điện thoại này chưa được đăng ký.');
      return;
    }
    setStep(2);
  };
  ...
  const handleReset = async () => {
    ...
    setLoading(true);
    const ok = await resetPassword(phone, newPassword);
    setLoading(false);
    if (ok) setStep(3);
          ...
          <View style={styles.card}>
            {step === 1 && (
              <>
                ...
                <TouchableOpacity style={styles.btn} onPress={handleCheckPhone}>
            ...
            {step === 2 && (
              <>
                ...
                <Text style={styles.subtitle}>Tài khoản: {phone}</Text>
